...
//nodeNum表示结点个数
//arcNum表示边的个数
void createGraph(HeadNode *G, /*int nodeNum, int arcNum, */ ifstream &ifs) {
	int nodeNum, arcNum;
	ifs >> nodeNum >> arcNum;
...
int main(int argc, char **argv) {
  ...
  if(argc == 2)
  	...
  	ifs.open("data.ini");
  createGraph(G, /* nodeNum, arcNum,*/ ifs);
  ...
  cout << "下面开始运行dijkstra算法..." << endl;
	...
	int start, endd;
	cout << "输入你想查找的送货源点和终点（0， 0代表你想结束程序）：";
	while(cin >> start >> endd && start != 0 && endd != 0){
		Dijkstra(G, nodeNum, start);
		cout << "到v" << endd << "的最短路径为" << G[endd-1].d << ": ";
		printPath(G, endd);
		cout << endl;
		cout << "输入你想查找的送货源点和终点（0， 0代表你想结束程序）：";
	}
